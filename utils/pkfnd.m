...
if nargin==3 && npks>0
   %throw out all pks within sz of boundary;
    ind1=find(mx(:,1)>sz & mx(:,1)<(nr-sz) & mx(:,2)>sz & mx(:,2)<(nc-sz));
    mx=mx(ind1,:); 
end
...
% function out = 
% 
% th = 3; %threshold the minimum brightness
% sz = 3; % if your data's noisy, then set this optional keyword to a value slightly larger than the diameter of your blob. if 
